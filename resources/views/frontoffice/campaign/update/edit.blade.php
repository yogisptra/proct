...
@section('content')
    ...
    <main class="mt-18">
        <section class="maxview align-items-start bg-white px-ss-2 pt-6 pb-8">
            <div class="container">
                <form action="{{ route('campaign-update.update', $data->id) }}" enctype="multipart/form-data" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="campaign_id" value="{{ $data->hasCampaign->id }}">
                    <div>
                        ...
                        <div class="position-relative"><i class="position-absolute left-4 top-3_5 text-default rck ryd-campaign {{ $errors->has('title') ? 'text-danger' : 'text-primary' }}"></i>
                            <input class="input ps-12" name="title" value="{{ $data->title }}" id="title" type="text" placeholder="Bantuan Telah Disalurkan" required>
                        </div>
                        @if ($errors->has('title'))
