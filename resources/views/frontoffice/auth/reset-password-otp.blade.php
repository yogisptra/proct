...
<main class="mt-18">
    <section class="maxview bg-white px-ss-2 pt-8 pb-8">
        <div class="container">
            ...
            <input type="text" hidden value="{{ $data->hasUserEmail->email ?? $data->hasUserPhone->email }}" id="valEmail">
            <form class="mt-6" action="{{ @$data->hasUserEmail ? route('auth-user.reset_confirmationOtp', @$data->hasUserEmail->id) : route('auth-user.reset_confirmationOtp', @$data->hasUserPhone->id) }}" method="POST">
                @csrf
            ...
            <div class="mt-6 text-center sendEmail">
                <a class="link-primary" href="{{ @$data->hasUserEmail ? route('auth-user.sendEmail', @$data->hasUserEmail->id) : route('auth-user.sendEmail', @$data->hasUserPhone->id) }}">Kirim Ulang Kode</a>
            </div>
        </div>
    </section>
</main>
