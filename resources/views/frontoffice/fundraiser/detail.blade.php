    ...
    <nav class="navbar transition-all position-fixed w-full top-0 left-0 z-1050 navbar--transparent">
        <div class="container">
            <div class="h-18 d-flex justify-content-between align-items-center position-relative">
                <div class="navbar__left d-flex align-items-center"><a class="d-flex align-items-center text-white me-4 transition-all" href="javascript:history.go(-1)" aria-label="Go Back"><i class="text-2xl rck ryd-arrow-left"></i></a><span class="line-clamp-1 text-base fw-medium text-default transition-all opacity-0 invisible">{{ @$data->title }}</span>
                </div>
    ...
    <main class="campaign-detail">
        <section class="bg-white pb-6">
            <div class="position-relative bg-skeleton w-full">
                <img class="w-full" src="{{ asset('frontoffice/assets/uploads/images/campaign-1.webp') }}" alt="{{ $data->title }}">
            ...
            <div class="container">
                <div class="pt-4">
                    ...
                    <div class="d-flex align-items-center justify-content-between text-default mt-2"> 
                        <span class="rp ns fw-medium text-secondary">{{ number_format($data->collected,0,'.','.') }}</span>
                        @if($data->open_goal == 1)
                    ...
                    <a class="campaign-detail__main-cta link-btn link-btn-primary h-12 w-full rounded-xs fw-medium mt-8" href="{{ route('frontoffice.fundraiser-payment', [ 'slug' => $data->slug, 'token' => $fundraiser->slug_token ]) }}"> <span>Donasi Sekarang</span><i class="text-default ms-2 rck ryd-arrow-right"></i>
                    </a>
                </div>
            </div>
        </section>
        <div class="floating-btn floating-btn--hide z-100">
            <div class="container bg-white pb-4"><a class="link-btn link-btn-primary h-12 w-full rounded-xs fw-medium" href="{{ route('frontoffice.fundraiser-payment', ['slug' => $data->slug, 'token' => $fundraiser->slug_token ]) }}"> <span>Donasi Sekarang</span><i class="text-default ms-2 rck ryd-arrow-right"> </i></a>
            </div>
        </div>
        <section class="bg-white py-4 mt-2">
            <div class="container">
                ...
                <a class="text-base d-flex align-items-center" href="{{ route('frontoffice.campaigner', ['type' => 'PERSONAL', 'id' => $data->encodeHash($data->hasUser->id)]) }}">
                    <div class="bg-skeleton w-8 h-8 rounded-md overflow-hidden shadow">
                        <img class="w-full h-full object-cover" src="{{ $data->donatur_id ? asset('assets/images/donatur/'. $data->hasUser->image) : asset('frontoffice/assets/uploads/images/no-avatar.svg') }}" alt="{{ $data->name }}">
                    </div>
                ...
                <a class="text-base d-flex align-items-center" href="{{ route('frontoffice.campaigner', ['type' => 'CORPORATE', 'id' => $data->encodeHash($data->hasUser->hasCorporate->id)]) }}">
                    <div class="bg-skeleton w-8 h-8 rounded-md overflow-hidden shadow">
                        <img class="w-full h-full object-cover" src="{{ asset('assets/images/corporate/'. @$data->hasUser->hasCorporate->image) }}" alt="{{ @$data->hasUser->hasCorporate->corporate_name }}">
                    </div>
                    <div class="d-flex align-items-center ms-2 flex-1">
        ...
        <section class="bg-white py-4 mt-2">
            <div class="container">
                ...
                <ul class="campaign-detail__updates h-60 overflow-hidden">
                    <li>
                        @forelse($programUpdate->take(1) as $row)
                        <div class="text-base d-flex align-items-center mb-4">
                            ...
                            <div class="bg-skeleton w-10 h-10 rounded-md overflow-hidden shadow">
                                <img class="w-full h-full object-cover" src="{{ $row->donatur_id ? asset('assets/images/donatur/'. $row->hasUser->image) : asset('frontoffice/assets/uploads/images/no-avatar.svg') }}" alt="{{ $row->name }}">
                            </div>
                            <div class="ms-3"><a class="link-base d-flex align-items-center flex-1" href="{{ route('frontoffice.campaigner', ['type' => 'PERSONAL', 'id' => $data->encodeHash($data->hasUser->id)]) }}"><span class="fw-medium line-clamp-1">{{ $row->hasCampaign->hasUser->name }}</span><i class="ms-1 mt-0_25 text-primary rck ryd-verified-simple"></i></a>
                            ...
                            <div class="ms-3"><a class="link-base d-flex align-items-center flex-1" href="{{ route('frontoffice.campaigner', ['type' => 'CORPORATE', 'id' => $data->encodeHash($data->hasUser->id)]) }}"><span class="fw-medium line-clamp-1">{{ $row->hasCampaign->hasUser->hasCorporate->corporate_name }}</span><i class="ms-1 mt-0_25 text-primary rck ryd-verified-simple"></i></a>
                                <time class="text-secondary fw-medium text-xs">{{ \Carbon\Carbon::parse($row->created_at)->format('d F Y') }}</time>
                            </div>
                            @endif
                        </div>
        ...
        <section class="bg-white py-4 mt-2">
            <div class="container">
                <div class="d-flex align-items-center justify-content-between mb-4">
                    ...
                    </div><span class="text-end">{{ count($allFundraiser ?? 0) }} Fundraiser</span>
                </div>
                ...
                @endif
                @if(isset($allFundraiser))
                <ul class="campaign-detail__fundraiser my-4">
                    @foreach($allFundraiser->take(1) as $row)
                    <li class="border border-gray-light rounded-xs p-4">
                        <div class="d-flex">
                            <div class="bg-skeleton w-12 h-12 rounded-circle overflow-hidden shadow">
                                <img class="w-full h-full object-cover" src="{{ $row->fundraiser_id ? asset('assets/images/donatur/'. $row->hasFundraiser->image) : asset('frontoffice/assets/uploads/images/no-avatar.svg') }}" alt="{{ $row->hasFundraiser->name }}">
                            </div>
                    ...
                    <li class="border border-gray-light rounded-xs p-4">
                        <div class="d-flex">
                            ...
                            <div class="flex-1 ms-3 mt-0_75">
                                ...
                                <button class="love-btn d-flex align-items-center mt-2 link-base-light"><i class="transition-all rck ryd-heart"></i><span class="ms-2 text-xxs">Aminkan doa</span>
                                </button>
                            </div>
                        </div>
                    </li>
